...
@section('title', 'Edit Design Template - Fazztrack')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600">Update template information and files</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('design-templates.update', $template) }}" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="category" id="category" 
                                ...
                                <option value="T-Shirt" {{ old('category', $template->category) == 'T-Shirt' ? 'selected' : '' }}>T-Shirt</option>
                                <option value="Hoodie" {{ old('category', $template->category) == 'Hoodie' ? 'selected' : '' }}>Hoodie</option>
                                <option value="Cap" {{ old('category', $template->category) == 'Cap' ? 'selected' : '' }}>Cap</option>
                                <option value="Polo" {{ old('category', $template->category) == 'Polo' ? 'selected' : '' }}>Polo</option>
                                <option value="Jacket" {{ old('category', $template->category) == 'Jacket' ? 'selected' : '' }}>Jacket</option>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="description" id="description" rows="3" 
                                  class="block w-full border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500"
                                  placeholder="Describe the template, its features, and usage...">{{ old('description', $template->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" name="tags" id="tags" value="{{ old('tags', $template->tags) }}" 
                               class="block w-full border-gray-300 rounded-lg focus:ring-primary-500 focus:border-primary-500"
                ...
                <!-- Add New Files -->
                <div>
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Add New Files</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="flex items-center justify-center w-full">
                                <label for="template_files" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                    ...
                                    <input id="template_files" name="template_files[]" type="file" class="hidden" multiple accept=".jpg,.jpeg,.png,.pdf,.psd,.ai,.zip">
                                </label>
                            </div>
